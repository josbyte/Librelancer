        ...
        RunSync(() =>
        {
            ...
            if (explode)
            {
                    ...
                    despawn.TryGetComponent<CDeployedMunitionComponent>(out var projectile))
                {
                    ...
                    {
                        // Resolve mine explosions lazily as a fallback for
                        // equipment databases initialized before the effect
                        // collection finished loading.
                        explosionFx = Game.GameData.Items.Effects.Get(effectName);
                    }

                    if (projectile is CMissileComponent missile)
                    {
...
#pragma warning restore CS8670
    ...
    void IClientPlayer.SpawnMissile(int id, bool playSound, uint equip, Vector3 position, Quaternion orientation)
    {
        RunSync(() =>
        {
            ...
            var go = new GameObject(eq.ModelFile.LoadFile(Game.ResourceManager)!,
