    ...
    public class ServerWorld
    {
            ...
            void HandleMissileCollision(GameObject projectile, GameObject other)
            {
                ...
                if (msl?.Target == other)
                {
                    ExplodeMissile(projectile);
                         ...
                         deployable.IsMine && !deployable.IsCollisionSafe &&
                         !deployable.IsOwner(other) && other.Kind == GameObjectKind.Ship)
                {
        ...
        public bool TryDivertMissile(SMissileComponent missile)
        {
            ...
            foreach (var obj in GameWorld.Objects)
            {
                ...
                var distance = Vector3.Distance(missilePosition, obj.PhysicsComponent.Body.Position);
                if (distance > countermeasure.Range)
                {
                    continue;
                }
            ...
            GameObject owner, bool isMine)
        {
            actions.Enqueue(() =>
            {
                ...
                var go = new GameObject(munition.ModelFile.LoadFile(Server.Resources)!, Server.Resources, false, true)
                {
                ...
                go.AddComponent(new SDeployableComponent(go, munition, owner, isMine));
                ...
                physics.Body.LinearVelocity = owner.PhysicsComponent.Body.LinearVelocity + launchVelocity;
                updatingObjects.Add(go);
